...
szStationOp = 
	{
		...
		"Maps luyÖn c«ng/goto_luyencong", 			--4
		"§i Hoa s¬n c¶nh kü tr­êng/go_HSBattle", 	-- 5
		"§i ChiÕn tr­êng c«ng thµnh/GoCityWar",			-- 6
		"§i chiÕn tr­êng huÊn luyÖn/GoDemoWar",		-- 7
		"§i ®¶o TÈy Tñy/GotoClearSkillMap",	-- 8
		"§i ®Õn n¬i lµm nhiÖm vô D· TÈu/tl_moveToTaskMap",			-- 9
		"§i n¬i ®Æc biÖt lµm NhiÖm vô TÝn Sø /messenger_wagoner",      -- 10
		--"È¥ÎÞÃû¹È/GotoAnonymVale"			--11
	};
...
aryCondition = {
	...
	[5] = {checkState = return_1}, 
	[6] = {checkState = function()                      -- È¥¹¥³ÇÕ½³¡
		return (WhichWarBegin()~= 0 and 1) or 0;
		   end;
		  },
	[7] = {checkState = function()                      -- È¥¹¥³ÇÑµÁ·Õ½³¡
				return (GetGlbValue(2)==1 and 1) or 0;
		   end;
		  },
	[8] = {checkState = return_1},     -- È¥Ï´Ëèµº
	[9] = {checkState = function()                      -- È¥Ò°ÛÅÈÎÎñËùÔÚµØ
				return (GetTask(1021)==4 and 1) or 0;
		   end;
		  },
	[10] = {checkState = function()                      -- È¥ÌØÊâÐÅÊ¹ÈÎÎñËùÔÚµØµã
				return (GetTask(1204)~= 0 and 1) or 0;
		   end;
		  },
	[11] = {checkState = return_1},     -- Â·¾­µÄµØ·½
	[12] = {checkState = return_1},
	[13] = {checkState = return_1},
}
...
CITY_OPTIONS =
{
	...
	{"§i ®Õn n¬i lµm nhiÖm vô D· TÈu",		tl_moveToTaskMap},		-- 9
	{"§i n¬i ®Æc biÖt lµm NhiÖm vô TÝn Sø ",	messenger_wagoner},     -- 10
	--"È¥ÎÞÃû¹È",				GotoAnonymVale"
	{"Xin h·y ®­a ta ®Õn ChiÕn Long §éng",		OnGoToNewLiangShuiDong},-- 11
	--{"§i ThÝ LuyÖn Cèc",				goto_shijiangu},
	{"§i ¸c Lang Cèc",				_GoELangGu}, --12
}
...
--	check_update()					-- ¼¼ÄÜ¸üÐÂ¡¢ÃÅÅÉ¼Ó±êÊ¶£¨2004-05-31£©
	if (GetLevel() >= 5) then
		if (WhichWarBegin() ~= 0) then
			Say(szMsg, 5, szStationOp[1], szStationOp[2], szStationOp[3], szStationOp[4], szStationOp[6], szStationOp_Cancel)
		elseif (GetGlbValue(2) == 1) then -- ¹¥³ÇÑµÁ·Õ½³¡
			Say(szMsg, 5, szStationOp[1], szStationOp[2], szStationOp[3], szStationOp[4], szStationOp[7], szStationOp_Cancel)
		else
			if (GetTask(1021) == 4) then -- Èç¹û½ÓÊÜÁËÒ°ÛÅÈÎÎñÖÐÑ°ÕÒµØÍ¼Ö¾µÄÈÎÎñ
				Say(szMsg, 5, szStationOp[1], szStationOp[2], szStationOp[3], szStationOp[4], szStationOp[9], szStationOp_Cancel)
			else
				Say(szMsg, 4, szStationOp[1], szStationOp[2], szStationOp[3], szStationOp[4], szStationOp_Cancel)
			end
		end;
	else		
