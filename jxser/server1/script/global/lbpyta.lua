...
Include("\\script\\global\\vldoxanh\\lastnpctalk.lua");
Include("\\script\\item\\doxanh\\testfuntion.lua");
Include("\\script\\item\\doxanh\\nangcapdoxanh.lua");
...
function main()
	dofile("script/global/gm/lenhbaitestserver_old.lua")
		...
		local tbSay = {format(TITLEDIALOG, GetName(), GetAccount() ,nW,nX,nY)};
				...
				tinsert(tbSay, "Qu¶n lý trang bÞ xanh/controlblueitem");
...
function controlblueitem()
	local nW,nX,nY = GetWorldPos()
	local szIP = GetIP()
	local tbSay = {format(TITLEDIALOG, GetName(), GetAccount() ,nW,nX,nY,szIP)}
	tinsert(tbSay, "Check Xanh/testxanhadd")
	tinsert(tbSay, "Check trang bÞ Xanh/checkdoxanhitem")
	tinsert(tbSay, "N©ng cÊp trang bÞ Xanh/nangcapdoxanh")
...
function testxanhadd()
AddQualityItem(0, 0, 2, 0, 10, 0, 0, 85, 121, 89, 440, 58, 134)
end
