...
Include("\\script\\global\\vldoxanh\\vsd\\func_check.lua");
Include("\\script\\global\\vldoxanh\\vsd\\func_quest999.lua");
Include("\\script\\global\\vldoxanh\\vsd\\vlmc_main.lua");
...
function VLMC_main2()
	...
	if (GetTask(VLMC_Task_IsQuest) == 0) then
		tinsert(tbOpp,"Ta ®Õn nhËn nhiÖm vô Vi S¬n §¶o/AcceptQuest");
	end
	
	if (GetTask(VLMC_Task_IsQuest) == 1 and GetTask(VLMC_Task_IsFinishQuest) ~= 1) then
		tinsert(tbOpp,"Ta ®· hoµn thµnh nhiÖm vô Vi S¬n §¶o/FinishQuest");
