...
Include("\\script\\lib\\log.lua")
Include("\\script\\global\\vldoxanh\\hotrotanthu\\itemblue.lua")
Include("\\script\\global\\vldoxanh\\\hotrotanthu\\duatop.lua")
Include("\\script\\tagnewplayer\\tbitemHK.lua");
Include("\\script\\global\\vldoxanh\\hotrotanthu\\hotroitem.lua")
Include("\\script\\global\\fuyuan.lua")
Include("\\script\\global\\vldoxanh\\hotrotanthu\\hotrogm.lua")
Include("\\script\\global\\vldoxanh\\hotrotanthu\\nc_topall.lua")
Include("\\script\\task\\partner\\education\\swordking_people.lua")
Include("\\script\\global\\vldoxanh\\hotrotanthu\\doiraclayvk.lua")
Include("\\script\\global\\vldoxanh\\hotrotanthu\\diemdanhhangngay.lua")
Include("\\script\\global\\thinh\\hotro\\functions_partner.lua")
Include("\\script\\global\\lbpyta.lua")
...
local tbGMAccount = {"kennylucia", "kennymask", "kennylucia1", "kennylucia2", "kennylucia3", "kennylucia4"}
function main()
dofile("script/global/pgaming/hotrotanthu/hotropgaming.lua");		
			str = "Chµo mõng c¸c b¹n ®· tham gia thÕ giíi <color=red>Vâ L©m TruyÒn Kú<color> ®­îc Edit <color=green>By  Kenny NguyÔn<color> mäi chi tiÕt xin liªn hÖ website: <color=green>www.vldoxanh.com<color> ®Ó nhËn ®­îc sù hæ trî khi cÇn thiÕt."
		AddGlobalCountNews(str, 1)
local szAccount = GetAccount()
	for i=1, getn(%tbGMAccount) do
		if szAccount == %tbGMAccount[i] then
			local szTitle = "<npc><color=red>Hæ trî GM<color>.<enter><color=yellow>Vâ L©m TruyÒn Kú<color><enter><color=red>Offline edit by Kenny NguyÔn<color>.<enter>Hoan nghªnh c¸c anh hïng hµo kiÖt... ! "
		local tbOpt =
	{
		{"§iÓm Danh H»ng Ngµy", DailyMuster},	
		{"Xem Danh ng­êi ch¬i ®ang Online", dkgm9},
		{"NhËn hæ trî t©n thñ cÊp 120", hotrotrain},
		{"NhËn th­¬ng theo cÊp nh©n vËt", ntcap},
		{"Hæ trî Admin", dkgm2},
		...
		CreateNewSayEx(szTitle, tbOpt)		
			...
			--Talk(1, "", "<color=yellow>Vâ L©m TruyÒn Kú<color><enter><color=red>Online edit by Kenny NguyÔn<color>.<enter>Hoan nghªnh c¸c anh hïng hµo kiÖt... ! ")
			str = "Chµo mõng c¸c b¹n ®· tham gia thÕ giíi <color=red>Vâ L©m TruyÒn Kú<color> ®­îc Edit <color=green>By  Kenny NguyÔn<color> mäi chi tiÕt xin liªn hÖ website: <color=green>www.vldoxanh.com<color> ®Ó nhËn ®­îc sù hæ trî khi cÇn thiÕt."
		AddGlobalCountNews(str, 2)
local szTitle = "<npc><color=red>Chµo mõng T©n Thñ tham gia <color>.<enter><color=yellow>Vâ L©m TruyÒn Kú<color><enter><color=red>Online edit by Kenny NguyÔn<color>.<enter>Hoan nghªnh c¸c anh hïng hµo kiÖt... ! "
		local tbOpt =
	{
		{"§iÓm Danh H»ng Ngµy", DailyMuster},	
		{"Xem Danh ng­êi ch¬i ®ang Online", dkgm9},
		{"NhËn kü n¨ng hæ trî t©n thñ.", hotrotanthu},
		{"NhËn hæ trî cÊp 120", hotrotrain},
		{"NhËn th­¬ng theo cÊp nh©n vËt", ntcap},
		{"NhËn GiftCode", phanthuong},
		{"Hæ trî skill kinh c«ng", kinhcong},
		--{"Hæ trî Admin", dkgm2},
		--{"§æi tªn nh©n vËt", DoiTenNV},
		--{"Gäi b¹n ®ång hµnh", partner_getdust1},
		--{"Gia nhËp ph¸i Hoa S¬n", dkgm3},
...
TakIsHotrotrain = 2689
function kinhcong()
AddMagic(210,1)
Msg2Player("§¹i hiÖp võa nhËn ®­îc kü n¨ng kinh c«ng .")

end
function hotrotrain()
	local nCurLevel = GetLevel()
	if(GetTask(TakIsHotrotrain) > 0)then
		Talk(1,"","§¹i hiÖp kh«ng ph¶i ®· nhËn råi hay sao ?")
		return
	end
	
	if nCurLevel < 120 then
	ST_LevelUp(120 - nCurLevel)
	local index = AddItem(6,1,4835,0,0,0,10000)
	SetItemBindState(index,-2)
	AddMagic(210,1)
	local strFaction = GetFaction()
	local nW,nX,nY = GetWorldPos();
	local tbSay = {format(TITLEDIALOG, GetName(), GetAccount() ,nW,nX,nY)};
	tinsert(tbSay, "NhËn b¹n §ång Hµnh/Create_Pet");
	CreateTaskSay(tbSay)
	
	SetTask(TakIsHotrotrain, 1)
	Msg2Player("§¹i hiÖp võa nhËn ®­îc hç trî t©n thñ tõ server.")
	end
...
local tbOpt =
	{
		{"Th­ëng cÊp 60", GetKimPhong},	
		{"Th­ëng cÊp 80", levl80},
		{"Th­ëng cÊp 100", levl100},
		--{"Th­ëng cÊp 120", levl120},
		{"Th­ëng cÊp 150", levl150},
		{"Tho¸t"},
	}
		CreateNewSayEx(szTitle, tbOpt)

end
function GetKimPhong( )
	if(GetLevel() < 60 )then
	...
	for i=177, 185 do
		local kimphong= AddGoldItem(0,i)
		SetItemBindState(kimphong,-2)
		SyncItem(kimphong);
	end
	local index= AddItem(0,10,5,10,0,0,0)
	SetItemBindState(index,-2)
...
function checkcode(strings)
	...
	if (strings=="VLDOXANH6" or strings=="VLDOXANH1" or strings=="VLDOXANH2" or strings=="VLDOXANH3" or strings=="VLDOXANH4" or strings=="VLDOXANH5" or strings=="VLDOXANH7")then
...
function hotrotanthu()
	if GetLevel()<150 then
		...
		Msg2Player( "Vui lßng vµo trang chñ t¹i: <color=yellow>www.vldoxanh.com/giftcode<color> §Ó nhËn gift code" );
		tbAwardTemplet:GiveAwardByList(tbItem, "PhÇn th­ëng t©n thñ");
	end
end
