...
dofile("script/global/namcung/topallserver/nc_topall.lua")
	...
	--if self.CheckDay() == 1 then
		...
		--tinsert(tbOpt, {"Ta muèn cËp nhËt th«ng tin", tbTop10.UpdateTopList, {tbTop10}})
	--end	
	tinsert(tbOpt, {"Xem danh s¸ch ThËp §¹i Cao Thñ", tbTop10.ShowTopList, {tbTop10}})
	tinsert(tbOpt, {"Hñy bá "})
	CreateNewSayEx("Ta phô tr¸ch viÖc ph¸t th­ëng thËp ®¹i cao thñ, ng­¬i cã muèn nhËn th­ëng kh«ng?", tbOpt)
end

function tbTop10:XepHangTop(n,ten,cap)
	local nResultHandle = OB_Create()
	OB_PushInt(nResultHandle, n) 
	OB_PushInt(nResultHandle, 0)
	OB_PushString(nResultHandle, ten)
	OB_PushInt(nResultHandle, cap)
	SendScriptData(2, nResultHandle)
	OB_Release(nResultHandle)
end
function tbTop10:hangcanhan()
	if GetLevel()<10 then
		return
	end
	if self.CheckTime() == 1 then
	...
	for nRank = 1, getn(self.tbTopList) do
		...
		if strName == GetName() then
			SetTask(3002,nRank)			-- Lay hang
			SetTask(3003,self.tbTopList[nRank][3])		-- Lay EXP
			SetTask(3004,self.tbTopList[nRank][1])
		end
	
		if nRank<10 then
			--tbTop10:XepHangTop(10287,self.tbTopList[nRank][1],self.tbTopList[nRank][2])	--thap dai cao thu
			--tbTop10:XepHangTop(10282,self.tbTopList[nRank][1],self.tbTopList[nRank][2])	--Thuy yen
		end
		--Msg2Player("H¹ng :"..nRank.." -- "..self.tbTopList[nRank][3].."")

		--Msg2Player("H¹ng :"..nRank.." -- "..self.tbTopList[nRank][1].." --Exp va Cap: "..self.tbTopList[nRank][4].."")
	end
end
...
function tbTop10:Regist()
	...
	tinsert(tbSubcriber, GetLevel())
	--kinh nghiÖm hiÖn t¹i
	tinsert(tbSubcriber, GetExp())
	tinsert(tbSubcriber, GetFaction())
	if self:Subcribers2File() == 1 then
...
function tbTop10:checkphai(player_Faction)
...
dofile("script/global/namcung/topallserver/nc_topall.lua")
if GetLevel() < 10 or GetFaction() == "" then
Talk(1, "", "Xin Lçi ! C¸c h¹ ®½ng cÊp qu¸ thÊp h·y mau chãng ®i luyÖn tËp thªm råi quay l¹i t×m ta !...")
return
end			
	local ntem=0
	for i = 1, 10 do
	...
	local strTittle = format("<color=Green>%-5s%s<color>\n%s <color=red>%s<color>\n\n", "", "Danh s¸ch ThËp §¹i Cao Thñ Toµn Server", "\t\t\t\t\t\tThêi gian tæng kÕt:", self.tbTopList[1][6])
	strTittle = strTittle..format("%-5s%-17s%-7s%-10s\n", "H¹ng", "Tªn", "CÊp", "M«n ph¸i")
	for i = 1, ntem do
		local nten, ncap,nphai = Ladder_GetLadderInfo(10270, i)
		phai = tbTop10:checkphai(nphai)
		if (szname == GetName()) then
			strTittle = strTittle..format("<color=cyan>%-5s%-17s%-7s%-10s<color>\n", i, nten, ncap,phai)
		else
			strTittle = strTittle..format("<color=green>%-5s<color><color=orange>%-17s<color><color=yellow>%-7s<color><color=cyan>%-10s<color>\n", i, nten, ncap,phai)
		end
	end
	CreateNewSayEx(strTittle,{{"§ãng",OnCancel}})
end
...
function tbTop10:UpdateTopList()
	...
	for key, val in self.tbSubcribers do
		nExp = val[1]+val[2]
		tinsert(tbTemp, {key, val[1], val[2], nExp, val[3],strTime})
