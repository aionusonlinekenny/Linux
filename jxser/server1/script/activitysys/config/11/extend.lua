...
function pActivity:DropMaterialBagInRandom(nNpcIndex)
	...
	Msg2Player("NhËn ®­îc <color=white>Tói Nguyªn LiÖu ")
end
...
function pActivity:SelectMaterialDlg(nGoldSelect)
	local szTitle = format("<npc>Xin h·y lùa chän ®æi<color=green>%s<color> nguyªn liÖu", %tbITEM_GREEN_CLOTH.szName)
	local tbOpt = 
	{
...
function pActivity:SelectGoldDlg()
	local szTitle = format("<npc>Xin h·y lùa chän c¸ch ®æi: <color=green>%s<color>\n\n", %tbITEM_GREEN_CLOTH.szName)
	local tbText = {}
	local szGold = "%d v¹n l­îng"
	local szOptText = "C¸ch ®æi thø %d\t - "
	for i=1,3 do
		if i < 3 then
		...
		szTitle = format("%s C¸ch %d\t: Nguyªn liÖu + %s.\n", szTitle, i, tbText[i])
