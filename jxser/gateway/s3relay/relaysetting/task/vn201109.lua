...
function TaskShedule()
	...
	TaskCountLimit(0);
	OutputMsg("KHOI DONG LE QUOC KHANH VA TRUNG THU");
end
...
function TaskContent()
	...
	OutputMsg(format("Su dung phao hoa se tang kinh nghiem %d--%d", nDate, nTime));
	...
	if nDate == 201109 and nTime == 20 then
		...
		local tCity =	{{1, "Ph­îng T­êng"},{11, "Thµnh §«"},{162, "§¹i Lý"},{37, "BiÖn Kinh"},
										{78, "T­¬ng D­¬ng"},{80, "D­¬ng Ch©u"},{176, "L©m An"}};
		local nItem = random(1, 7);
		local strMsg = format("Tõ %d: %02d µ½ %d: %02d ÔÚ%s S÷ Dông Ph¸o Hoa Quèc Kh¸nh sÏ NhËn d­îc 10%% Kinh NgiÖm.", 
