-- Ñ×µÛµÄ±¦²Ø±ÈÈü¶¨Ê±Æ÷
--Ã¿ÌìÏÂÎç2µãµ½ÏÂÎç12µãÕûµã´¥·¢

--by Ğ¡ÀË¶à¶à
--2007.10.22
INTERVAL_TIME = 60	-- Ã¿Ğ¡Ê±´¥·¢
Include("\\script\\gb_modulefuncs.lua")
Include("\\script\\leaguematch\\timetable.lua")
Include("\\script\\leaguematch\\head.lua")

function GetNextTime()	--Ã¿ÌìÏÂÎç1µã55µ½ÍíÉÏ23µã55 ´¥·¢±¨Ãû
	-- ¸Ä±äÑ×µÛĞÔÄÜ²Î¼ÓµÄ±¨ÃûÊ±¼ä-Modifiled by AnhHH - 20110802
    local hour = tonumber(date("%H"));
    if (hour == 24) then
    	hour = 13;
    elseif(hour <= 14) then
    	hour = 14
    else
    	hour = hour + 1;
    end
    return hour, 25;
end

function TaskShedule()
	TaskName("YANDIBAOZANG");	
	
	-- 60·ÖÖÓÒ»´Î
	TaskInterval(INTERVAL_TIME);
	-- ÉèÖÃ´¥·¢Ê±¼ä
	local h, m = GetNextTime();
	TaskTime(h, m);
	OutputMsg(format("=================Hoat Dong Viem De Bat Dau %d:%d...=================", h, m));
	-- Ö´ĞĞÎŞÏŞ´Î
	TaskCountLimit(0);
	-- OutputMsg("Æô¶¯×Ô¶¯¹ö¶¯¹«¸æ...");
end

function TaskContent()
	local TB_YDBZ_DATE_START =	--Æô¶¯³¡´Î(Ê±)
	{
		14,16,18,20,22,
	}
	
	local nhour = tonumber(date("%H"))
	for nindex,ndate_hour in TB_YDBZ_DATE_START do
		-- ¸Ä±äÑ×µÛĞÔÄÜ²Î¼ÓµÄ±¨ÃûÊ±¼ä -Modifiled by AnhHH - 20110802
		if (nhour == ndate_hour) then
			
--			if gb_GetModule("YANDIBAOZANG") == 1 and gb_GetModule("YANDIBAOZANG_TALK") == 1 then
				OutputMsg("=================Hoat Dong Viem De"..tonumber(date("%H"))..":25 Bat Dau======");
				-- ´¥·¢GameServerÉÏµÄ½Å±¾
				--GlobalExecute("dw LoadScript([[\\settings\\trigger_challengeoftime.lua]])");
				GlobalExecute("dwf \\script\\missions\\yandibaozang\\yandibaozang_trigger.lua YDBZ_OnTrigger()");
				szMsg = "Viªm §Õ b¶o tµng më c÷a b¸o danh, h·y nhanh ch©n ®Õn B×nh C« N­¬ng ë BiÖn Kinh ®Ó tham gia, thêi gian ghi danh lµ 5 phót."
				GlobalExecute(format("dw AddLocalCountNews([[%s]], 2)", szMsg))
--			end
			break;
		end
	end
end

function GameSvrConnected(dwGameSvrIP)
end
function GameSvrReady(dwGameSvrIP)
end