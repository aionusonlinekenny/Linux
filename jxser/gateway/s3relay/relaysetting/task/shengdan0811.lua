...
function TaskShedule()
	...
	--…Ë÷√¥•∑¢¥Œ ˝£¨0±Ì æŒﬁœﬁ¥Œ ˝
	TaskCountLimit(0)
	local szMsg = format("=====%s BAT DAU %d:%d BAT DAU %d SE KET THUC=======", "HOAT DONG BAT LINH BO",nNextHour, nNextMinute, nInterval)
	OutputMsg(szMsg);
end
...
{nId = 99, szName = "V‹nh Lπc Tr n", tbPos = { {216, 182}, {185, 199} }},
		...
		for i=1, 1 do
			...
			local szMsg = format("C„ ng≠Íi th y 5 Æπi linh bﬂ xu t hi÷n<color=green>%s<color>, xin h∑y nhanh ch©n bæt l y!", tbMap[nIdx].szName)
	...
	elseif flag == 1 then
		local szMsg = "N®m Æπi linh bﬂ Æ∑ xÊng chuÂng, c∏c vﬁ lπi th´n tr n nhanh ch©n bæt l y."
		GlobalExecute(format("dw AddLocalCountNews([[%s]], 2)", szMsg))
		GlobalExecute(format("dw Msg2SubWorld([[%s]])", szMsg))
		OutputMsg(szMsg);
	elseif flag == 3 then
		...
		OutputMsg("THOI GIAN NHAN THUONG DA HET.");
	end
