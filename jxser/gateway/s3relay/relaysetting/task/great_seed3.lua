...
greatseed_configtab = {
	...
	--{318,2,100,"\\settings\\maps\\great_night\\³¤°×É½±±¢´.txt","Tr­êng B¹ch S¬n B¾c"},	-- ³¤°×É½±±¢´
	--{321,2,100,"\\settings\\maps\\great_night\\³¤°×É½ÄÏ.txt","Tr­êng B¹ch S¬n Nam"},		-- ³¤°×É½ÄÏ
	--{185,2,30,"\\settings\\maps\\great_night\\É³Ä®É½¶´1.txt","Sa m¹c S¬n §éng 1"},		-- ³¤°×É½ÄÏ
	--{186,2,30,"\\settings\\maps\\great_night\\É³Ä®É½¶´2.txt","Sa m¹c S¬n §éng 2"},		-- ³¤°×É½ÄÏ
	--{187,2,40,"\\settings\\maps\\great_night\\É³Ä®É½¶´3.txt","Sa m¹c S¬n §éng 3"},		-- ³¤°×É½ÄÏ
	
--	{182,1,25,"\\settings\\maps\\great_night\\ÄõÁú¶´ÃÔ¹¬.txt","NghiÖt Long §éng"}, -- qua hoang kim
--	{959,3,100,"\\settings\\maps\\great_night\\chienlongdong.txt","ChiÕn Long §éng"},	-- Qu¶ huy hoang
	...
	{53, 4, 3, "\\settings\\maps\\great_night\\balanghuyen.txt","Ba L¨ng HuyÖn"},			-- Ë«Áú¶´
...
function TaskShedule()
	...
	TaskCountLimit(0)
	OutputMsg("QUA HUY HOANG on start at 20 PM");
end
...
function TaskContent()
	...
	if righttime_add() ~= 1 then--Èç¹ûÊ±¼ä²»¶Ô
		...
		gb_SetTask("QUA HUY HOANG", 20, 0);	--12ºÅ±äÁ¿ÓÃÀ´´æ´¢NPCµÄÐòºÅ
		return
	end;
	...
	OutputMsg("=================QUA HUY HOANG on started at 20 PM=================")
...
--			szMsg = "H¹t Huy Hoµng TiÓu"
		...
		elseif (mod(nBatch,2) == 0) then
			szMsg = format("GÆp thêi tiÕt thuËn lîi <%s> ®· kÕt qu¶ t¹i %s, C¸c §¹i HiÖp h·y nhanh chãng thu ho¹ch!!",
			szMsg,
			greatseed_configtab[i][5]);
			GlobalExecute(format("dw AddLocalNews([[%s]])",szMsg));
		end
